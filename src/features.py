...
def load_mask(path: Path) -> np.ndarray:
    ...
    if np.all(foreground):
        raise ValueError(f"Mask contains no background pixels: {path}")
...
def extract_color_statistics(image_path: Path, mask_path: Path) -> dict[str, float]:
    ...
    for region_name, region_mask in regions.items():
        for feature_name, channel in data.items():
            values = channel[region_mask]
            statistics = _summary(values)

            for statistic_name, value in statistics.items():
                result[f"{region_name}_{feature_name}_{statistic_name}"] = value
